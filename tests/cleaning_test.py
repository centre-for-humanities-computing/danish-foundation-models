...
def test_stop_word():
    ...
    assert qfilter.stop_word(qfilter.nlp(texts[0]), n=2, stop_words=stop_words) is True
    assert qfilter.stop_word(qfilter.nlp(texts[1]), n=1, stop_words=stop_words) is False


def test_line_bullets_or_ellipsis():
    qfilter = QualityFilter()
    # bullet
    assert (
