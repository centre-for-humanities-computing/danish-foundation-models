...
for year in range(2006, 2017):
    ...
    mask = load_dataset(
        "json",
        data_files=f"/work/netarkivet-cleaned/deduplicated/{year}/mask.jsonl",
        streaming=True,
    )
    mask = mask["train"]
    ...
    for f in files:
        ...
        if "is_duplicate" in ds.features:
            ...
            ds = ds.add_column(
                "is_duplicate",
                [
                    next(iter_mask) if passed is True else None
                    for passed in ds["passed_quality_filter"]
                ],
            )
        ...
        n_clean_tokens_ = [
            n_tokens
            for n_tokens, is_dup in zip(ds["n_tokens"], ds["is_duplicate"])
            if is_dup is False
        ]
        ...
        assert (
            len(mask_) == n_passed_quality_filter_
        ), "number of deduplicated files does not match n_passed_quality_filter_"
...
msg.good("Finished. The following report some relevant statistics:")
