...
def apply_filter(dataset=Iterable[dict], columns_to_keep=["content"]) -> Iterable[dict]:
...
def main(
    netarkivet_path=Path(
        "/data-big-projects/danish-foundation-models/netarkivet_cleaned/v2"
    ),
    write_folder=Path("/data-big-projects/danish-foundation-models/netarkivet_cleaned"),
    version="2.0.0",
    buffer_size=1_000_000,
):
    for year in range(2006, 2017):
        msg.info(f"starting year: {year}")
        path = netarkivet_path / str(year) / "deduplicated" / "deduplicated" / "*.jsonl"
        write_path = write_folder / f"nat_{year}_v{version}.jsonl"
        if os.path.exists(write_path):
            msg.warn(f"\tFile already exists - skipping: {write_path}")
            continue

        jsonl_files = glob.glob(str(path))
        assert len(jsonl_files) > 0, f"No files found for year: {year}"
