...
for year in range(2006, 2017):
    ...
    j_files = sorted(j_files, key= lambda path: int(os.path.splitext(path)[0].split("/")[-1]))
    for f in j_files:
        ...
        ds = ds["train"]
        ds_subset = ds.remove_columns([c for c in ds.features.keys() if c not in {"text", "n_tokens", "is_duplicate", "passed_quality_filter", "timestamp", "language", "domain_key"}])
        ds_subset = ds_subset.map(word_count, batched = True, batch_size=1024*2*2, num_proc=32, remove_columns=["text"])
