...
class ELECTRAModel(PreTrainedModel):
    ...
    def forward(self, input_ids, is_mlm_applied, labels, attention_mask, token_type_ids):
        """
        ...
        """
        gen_logits = self.generator(input_ids, attention_mask, token_type_ids)[0] # (B, L, vocab size)
        ...
        return mlm_gen_logits, generated, disc_logits, is_replaced, attention_mask.to(bool), is_mlm_applied
...
class ElectraDataCollator(DataCollatorForLanguageModeling):
    ...
    def torch_call(self, examples: List[Union[List[int], Any, Dict[str, Any]]]) -> Dict[str, Any]:
        ...
        if self.mlm:
            batch["input_ids"], batch["labels"], batch["is_mlm_applied"] = self.torch_mask_tokens(
    ...
    def torch_mask_tokens(self, inputs, special_tokens_mask, mlm_probability=0.15, replace_prob=0.1, orginal_prob=0.1, ignore_index=-100):
        """ 
...
collator = ElectraDataCollator(tokenizer=hf_tokenizer,
                          mlm_probability=0.15,
                          replace_prob=0.1, 
                          orginal_prob=0.1,
                          for_electra=True)

electra_model = ELECTRAModel(generator, discriminator, PretrainedConfig(pad_token_id = hf_tokenizer.pad_token_id))
...
def group_texts(examples):
    tokenized_inputs = hf_tokenizer(
       examples["sentence"], return_special_tokens_mask=False, truncation=True, max_length=hf_tokenizer.model_max_length
    )
    return tokenized_inputs
# preprocess dataset
tokenized_dataset = dataset.map(group_texts, batched=True, remove_columns=["sentence", "idx", "label"])


trainer = ElectraTrainer(electra_model, tokenizer=hf_tokenizer, train_dataset=tokenized_dataset, data_collator=collator)
