...
def convert_nat_to_standard_format(nat_df: pl.LazyFrame) -> list[NatEntry]:
    ...
    nat_df = nat_df.select(
            ...
            pl.col("timestamp").str.strptime(dtype=pl.Datetime, format="%Y%m%d%H%M%S").dt.strftime("%Y-%m-%d %H:%M:%S.000Z").alias("created"),
...
if __name__ == "__main__":
    ...
    for path in get_nat_paths():
        ...
        part_id = path.with_suffix("").with_suffix("") # call twice to remove both .parquet and .snappy
