"""
Applied safesearch to a list of websites
...
from src.application.content_filter_lang_netarkivet import get_paths
...
def main():
    ...
    if os.path.isfile(previous_lookups):
        ...
        msg.warn(f"Did not find any previous lookups.")
        prev = {}
    ...
    counters = [Counter(read_json(path)) for path in paths]
    counter = sum_counters(counters)

    api_calls = daily_api_calls 
    # remove singles
    domains = [dom for dom, count in counter.most_common() if (count > 1 and dom not in already_checked)]
    ...
    n = len(domains)
    start, end = 0, 0
    msg.info(f"A total of {n} domains. Planning to make {len(range(500, n, 500))+1} API calls")
