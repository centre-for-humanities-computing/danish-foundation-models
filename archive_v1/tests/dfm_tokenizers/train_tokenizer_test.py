...
class TestTrainTokenizer:
    ...
    def dataset(self):
        return load_dataset("DDSC/lcc", split="test")

    @pytest.fixture(scope="class")
    def streamed_dataset(self):
        return load_dataset("DDSC/lcc", split="test", streaming=True)

    @pytest.fixture(scope="class")
    def train_params(self):
        return {"save_tokenizer": False, "show_progress": False}

    @pytest.fixture(scope="class")
    def test_docs(self):
        return ["Dette er en dårlig <mask>.", "Test…"]
    ...
    def test_iterable_of_texts(
        ...
        tok = train_tokenizer(
            corpus=(s["text"] for s in dataset),
            config=config,
            **train_params,
        )
